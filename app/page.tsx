...
export default function Home(){
  ...
  return <main>
    ...
    <section className="section" id="work"><div className="section-head"><p className="eyebrow">WORK</p><h2>Things I’m building.</h2><p>Projects, products and organizations connected to my work.</p></div><div className="cards">{projects.map(p=><article className="card" key={p.name}><div className="card-top"><span>{p.type}</span><b>{p.status}</b></div>{p.logo && <div className="project-logo"><img src={p.logo} alt={p.logoAlt}/></div>}<h3>{p.name}</h3><p>{p.desc}</p><a href={p.href} target="_blank" rel="noreferrer">Visit project ↗</a></article>)}</div></section>
